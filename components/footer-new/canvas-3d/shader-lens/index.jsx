...
const ShaderLens = ({dimensions}) => {
  ...
  const fragmentShader = `
        ...
        void main() {
            ...
            if (id > 0.5 && id < 1.5) {
              /* Draw if sphere hitten */
              vec3 pos = ro + t*rd; // Evaluate intersection point
              vec3 nor = normSphere(pos, sph1);

              // vec3 light = normalize(vec3(0.87703) - pos);
              // vec3 light = normalize(vec3(0.57703));

              /* Lights */
              vec3 lightDir = normalize(vec3(3.0, 4.0, -5.0));
              vec3 lightDir1 = normalize(vec3(3.0, 1.0, -5.0));
              vec3 viewDir = normalize(-rd);
              vec3 reflectDir = reflect(-lightDir, nor); // Reflection vector
              vec3 pointDir1 = normalize(vec3(1.0, 2.0, -2.0));

              /* Diffuse lighting */
              float diffuse = max(dot(nor, lightDir), 0.0);
              float diffuse1 = clamp(dot(nor, lightDir1), 0.0, 1.0);
              float ao = 0.5 + 0.5 * nor.y; // Ambient occlusion

              /* Specular highlight */
              float shininess = 32.0; // Sharpness control
              float specular1 = pow(max(dot(viewDir, reflectDir), 0.0), shininess);
              float specular2 = pow(clamp(dot(nor, pointDir1), 0.0, 1.0), 150.0);

              /* Rim light */
              float rim = pow(1.0 - max(dot(viewDir, nor), 0.0), 2.0); // Control border internsity
              vec3 rimColor = vec3(1.0); // Pure white
              float rimStrength = 0.5; // Border intensity

              /* Colors */
              vec3 baseColor = vec3(0.8, 0.7, 0.000);
              vec3 yellow = vec3(0.988, 0.890, 0.000); // Yellow
              vec3 lightYellow = vec3(0.1, 0.2, 0.4); // Yellow tint: white added to base color
              vec3 specularColor = vec3(1.0); // White underline
              vec3 ambientColor = vec3(0.1, 0.1, 0.1); // Ambient light

              /* Final color */
              color = baseColor 
                    + yellow * diffuse1 * ao
                    + vec3(0.0, 0.0, 1.0) * specular2 * 10.0
                    + rim * rimStrength * rimColor
                    + specular1;
            }
