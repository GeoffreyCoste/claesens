...
useEffect(() => {
    const updateContainerHeight = () => {
        if (!mobile) return; // Ne pas exécuter si mobile est false
        if (sectionRef.current && containerRef.current && contentRef.current) {
            const contentHeight = contentRef.current.offsetHeight;
            sectionRef.current.style.height = `${contentHeight + 200}px`;
            containerRef.current.style.height = `${contentHeight}px`;
    ...
    updateContainerHeight(); // Mettre à jour au montage si mobile est true
    window.addEventListener('resize', updateContainerHeight);
    return () => window.removeEventListener('resize', updateContainerHeight);
}, [mobile]); // Déclenchement du useEffect quand `mobile` change */
