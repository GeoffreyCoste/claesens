...
const PointsParticles = () => {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Random size
      sizes.push(Math.random() * 0.2 + 0.1); // Size between 0.1 and 0.3

      // Add color gradient or random colors
      colors.push(1, 0.8, 0); // RGB for yellow/orange
    }
    return {
  ...
  return (
    <Points
      ...
      frustumCulled={false}
    >
      <pointsMaterial size={0.01} vertexColors transparent opacity={0.8} />
    </Points>
  );
};
